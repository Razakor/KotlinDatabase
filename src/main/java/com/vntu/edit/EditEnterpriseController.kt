...
class EditEnterpriseController {
    ...
    private lateinit var enterpriseInfo: ArrayList<String>
    ...
    fun initData(enterpriseStateController: EnterpriseStateController, selectedRow: String) {
        ...
        enterpriseInfo = ArrayList(listOf(*selectedRow.split(", ".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()))
    ...
    fun editEnterprise() {
        ...
        val query = "UPDATE enterprise " +
                "SET name = '$name', `index` = '$index', adress = '$adress', telephone = '$telephone', fax = '$fax', email = '$email', website = '$website' " +
                "WHERE name = '${enterpriseInfo[2]}' AND adress = '${enterpriseInfo[1]}'"
